...
func _ready() -> void:
    ...
    for x in [-5.4, 5.4]:
        GeomUtil.static_box(
            ...
            "BreachWall",
            Vector3(x, 2.6, 23.5),
            Vector3(2.0, 5.2, 0.75),
            Color(0.16, 0.17, 0.16)
        )
    for stripe_i in 8:
        var marker := GeomUtil.box_mesh(
            Vector3(0.45, 0.02, 1.6),
            Color(0.80, 0.55, 0.09),
            0.88,
            0.04
        )
    ...
    for side in [-1.0, 1.0]:
        ...
        var screen := GeomUtil.box_mesh(
            Vector3(0.72, 0.46, 0.035),
            Color(0.15, 0.37, 0.34),
            0.28,
            0.22
        )
        screen.material_override = GeomUtil.emissive_material(
            Color(0.09, 0.30, 0.26),
            1.8,
            0.26,
            0.16
        )
